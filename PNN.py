...
class PNN():
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.sigmoid_out = tf.nn.sigmoid(self.out)
            self.loss = tf.losses.log_loss(self.label, self.sigmoid_out)
    ...
    def predict(self, Xi, Xv):
        ...
        out = self.sess.run([self.sigmoid_out], feed_dict=feed_dict)
        return out
    ...
    def fit(self,
        ...
        for epoch in range(self.epoch):
            ...
            for i in range(total_batch):
                ...
                t2 = time.time()
                loss, opt = self.sess.run([self.loss, self.optimizer], feed_dict=feed_dict)
                print(str(t2 - t1) + "s:" + " epoch:", epoch, "loss =", loss)
